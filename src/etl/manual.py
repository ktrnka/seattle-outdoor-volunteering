...
class ManualExtractor(BaseListExtractor):
    ...
    def __init__(self, raw_data: str):
        ...
        self.config = ManualEventsConfig.model_validate(yaml.safe_load(raw_data))

    @classmethod
    def fetch(cls) -> "ManualExtractor":
        """Load manual events from the YAML configuration file."""
        manual_events_path = Path(__file__).parent.parent.parent / "data" / "manual_events.yaml"

        if not manual_events_path.exists():
            raise FileNotFoundError(f"Manual events file not found: {manual_events_path}")
    ...
    def extract(self) -> List[Event]:
        ...
        for event_def in self.config.recurring_events:
            recurring_events = self._generate_recurring_events(event_def, today, end_date)
    ...
    def _generate_recurring_events(self, event_def: ManualEventDefinition, start_date: date, end_date: date) -> List[Event]:
        ...
        while current_date <= end_date:
            event_date = self._get_nth_weekday_of_month(current_date.year, current_date.month, event_def.recurring_pattern)
    ...
    def _create_event_instance(self, event_def: ManualEventDefinition, event_date: date) -> Event:
        ...
        start_utc = self._build_datetime(event_date, self._parse_time(event_def.start_time))
        end_utc = self._build_datetime(event_date, self._parse_time(event_def.end_time))
